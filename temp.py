...
mlpr = MLPRegressor(max_iter=2000, alpha=1, random_state=0, activation='relu').fit(
    X_train_scaled, y_train)

print(
    "The score of Nural Network/MLPRegressor:{:.2f}".format(mlpr.score(X_test_scaled, y_test)))

mlpr = MLPRegressor(max_iter=2000, alpha=1, random_state=0, activation='tanh').fit(
    X_train_scaled, y_train)
...
mlpr = MLPRegressor(max_iter=2000, alpha=1, random_state=0, activation='logistic').fit(
    X_train_scaled, y_train)
